...
const foto=document.getElementById('printFoto');
propic.addEventListener('change', checkSize);
foto.addEventListener('change', checkSize);
...
function checkSize (event){
    ...
    if(file.size>41943040){
        window.alert('Le dimensioni del file non possono superare 49 Mb')
        console.log(submit);
...
for(let article of articles){
    ...
    fetch("managementArea/dati/"+article.dataset.type).then(onResponse).then(onJsonCaricamento);
...
function addStampa(event){
    ...
    rows.shift();
    const op=fotoID.querySelectorAll('option');
    for(let o of op){
        o.remove();
    }
    for(let rw of rows){
...
function sendStampa(event){
    ...
    if((altezza.value!=0)&&(larghezza.value!=0)&&(materiale.value!=0)&&(prezzo.value!=0)&&(foto.value!=0)){
        ...
        add.addEventListener('click', addStampa);
        fetch("managementArea/addStampa/"+encodeURIComponent(String(altezza.value))+'/'+encodeURIComponent(String(larghezza.value))+'/'+encodeURIComponent(String(materiale.value))+'/'+encodeURIComponent(String(prezzo.value))+'/'+encodeURIComponent(String(foto.value))).then(onResponse).then(onJsonInsertStampa);
    }
    else{
...
function deleteFotografo(event){
    fetch("managementArea/removeFotografo/"+event.currentTarget.dataset.ID).then(onResponse).then(onJsonRemoveFotografo);
}
function deleteFoto(event){
    fetch("managementArea/removeFoto/"+event.currentTarget.dataset.ID).then(onResponse).then(onJsonRemoveFoto);
}
...
function deleteStampa(event){
    fetch("managementArea/removeStampa/"+event.currentTarget.dataset.IDS).then(onResponse).then(onJsonRemoveStampa);
}
