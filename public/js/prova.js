...
function removeCarrello(event){
    ...
    fetch("userArea/removeCart/"+encodeURIComponent(String(button.parentNode.parentNode.dataset.id))).then(onResponse).then(onJsonRemCart);
...
function onJsonAddOrder(promise){
    ...
    nOrdine.textContent="Ordine n. " + promise.items.ID_ACQUISTO;
    ...
    if(document.querySelector('#ordini .noOrder')){
        ...
        caricaOrdini(promise.items, div4);
    }
    else{
...
function buy(event){
    const button=event.currentTarget;
    fetch("userArea/addOrder/"+encodeURIComponent(String(button.parentNode.parentNode.dataset.stampa))).then(onResponse).then(onJsonAddOrder);
}
...
function onJsonAddCart(promise){
    ...
    if(document.querySelector('#carrello .noOrder')){
        ...
        div4.classList.add('content');
        carrello.appendChild(div4);
        caricaCarrello(promise.items, div4);
    }
    else{
...
function sendRecensione(id, element){
    ...
    if(voto!=0 && description!=''){
        ...
        fetch("userArea/addRec/"+encodeURIComponent(String(voto))+'/'+encodeURIComponent(String(description))+'/'+encodeURIComponent(String(id))).then(onResponse).then(onJsonAddRec);
    }
...
function addCart(event){
    const button=event.currentTarget;
    console.log('aggiungo al carrello la stampa'+button.parentNode.parentNode.dataset.stampa);
    fetch("userArea/addCart/"+encodeURIComponent(String(button.parentNode.parentNode.dataset.stampa))).then(onResponse).then(onJsonAddCart);
}
...
function removePref(event){
    const button=event.currentTarget;
    fetch("userArea/removePref/"+encodeURIComponent(String(button.parentNode.parentNode.dataset.id))).then(onResponse).then(onJsonRemPref);
    button.parentNode.parentNode.remove();
}
function caricaPreferiti(items, div){
    for(let item of items){
        ...
        console.log('preferito: '+item.ID_PREF+'stampa'+item.stampa);
...
function caricaRecensioni(item, div){
...
fetch("userArea/recensioni").then(onResponse).then(onJsonRecensioni);
fetch("userArea/carrello").then(onResponse).then(onJsonCarrello);
fetch("userArea/ordini").then(onResponse).then(onJsonOrdini);
fetch("userArea/user").then(onResponse).then(onJsonUser);
fetch("userArea/preferiti").then(onResponse).then(onJsonPreferiti);